<x-jet-action-section>
    <x-slot name="title">
        {{ __('Authentication à double facteur') }}
    </x-slot>
    ...
    <x-slot name="content">
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
            {{ __("Vous avez activé l'auhentification à double facteur.") }}
            @else
            {{ __("Vous n'avez pas activer l'authentification à double facteur.") }}
            @endif
        </h3>
